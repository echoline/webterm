...
function newWindow(id, canclose) {
	...
	mkfile("/dev/hsys/" + id + "/cons", undefined, function(f, p) {
			try {
				respond(p, "");
			/*	div.terminal.readterminal(p.count, function(l) {
					respond(p, l);
				}, p.tag);*/
			} catch(err) {
		...
		function(f, p) {
			...
			/*	var i;
				for (i = 0; i < p.data.length; i++)
					div.terminal.addchar(p.charCodeAt(i));*/
			//	div.terminal.writeterminal(p.data);
	...
	mkfile("/dev/hsys/" + id + "/complete", function(f) {
		...
		function(f, p) {
			try {
				...
				while(div.terminal.backlog.charAt(div.terminal.backlog.length-1) != ' ') {
					k.writestr('\b');
					div.terminal.backlog = div.terminal.backlog.substring(0, div.terminal.backlog.length-1);
				}
				k.writestr(toutf8(p.data));
				respond(p, -1);
			} catch (e) {
