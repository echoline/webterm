...
function newWindow(id, canclose) {
	...
	mkfile("/dev/hsys/" + id + "/complete", function(f) {
		...
		function(f, p) {
			if (!div.terminal.rawmode) {
				var consbuf = div.terminal.consbuf;
				var i = consbuf.lastIndexOf(' ');
				i++;
				div.terminal.consbuf = consbuf.substring(0, i) + toutf8(p.data);
				div.terminal.writeterminal("");
			}
			respond(p, -1);
		},
		function(f) {
