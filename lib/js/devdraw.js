function devdraw(winid) {
	...
	const top = mkdir("/dev/hsys/" + winid + "/draw");
	const _new = mkfile("/dev/hsys/" + winid + "/draw/new", undefined, function(f, p) {respond(p, "");}, function(f, p) { respond(p, -1);});
	var ids;
	...
	function resize(w, h) {
		...
		canvas.width = w;
		canvas.height = h;
		displayRect.max.x = canvas.width;
		displayRect.max.y = canvas.height;
		if(display)
			display.free();
		display = new Image({
			id: 0,
	...
	const winname = mkfile("/dev/hsys/" + winid + "/winname");
	winname.read = function(fid, p) {
		readstr(p, display.name);
	}
	...
	_new.open = function(fid) {
		...
		if (!canvas) {
			...
			canvas.width = parseInt(win(winid).bg.style.width.replace(/px$/,''));
			canvas.height = parseInt(win(winid).bg.style.height.replace(/px$/,''));
			canvas.oncontextmenu = function(event) { return false; };
			canvas.onkeydown = function(event) { win(winid).terminal.onkeydown(event); };
			win(winid).bg.appendChild(canvas);
			ctx = canvas.getContext('2d');

			displayRect = {min: {x: 0, y: 0}, max: {x: canvas.width, y: canvas.height}};
	
			resize(canvas.width, canvas.height);
		}
		oshow(winid, false);
		var num = ids++;
		var dir = mkdir("/dev/hsys/" + winid + "/draw/" + num);
		var ctl = mkfile("/dev/hsys/" + winid + "/draw/" + num + "/ctl");
		fid.f = ctl;
		var data = mkfile("/dev/hsys/" + winid + "/draw/" + num + "/data");
		var colormap = mkfile("/dev/hsys/" + winid + "/draw/" + num + "/colormap");
		var refresh = mkfile("/dev/hsys/" + winid + "/draw/" + num + "/refresh");

		refresh.read = invalidop;
		...
		ctl.open = function(f) { nctl++; }

		ctl.read = function(fid, p){
		...
		ctl.write = function(fid, p){
			error9p(p, "no.");
		};

		ctl.close = function(f) {
			nctl--;
			if (nctl == 0) {
				display.free();
		...
		data.read = function(f, p) {
			readstr(p, "no.");
		}
		data.write = function(fid, p){
			let b = new VBuffer(str2arr(p.data));
			var m, e;
			while(b.rp < b.p){
				let n = b.rp;
				try{
